...
def train(_class_):
    ...
    for epoch in range(epochs):
        ...
        print(f"epoch: {epoch}  train_time:{int(time.time() - start_time)}  train_loss:{np.mean(loss_list)}")
        if (epoch + 1) % 20 == 0 :
            ckp_path = f'./checkpoints/mvtec/mvtec_DINL_{_class_}_{epoch}_{cnt}.pth'
